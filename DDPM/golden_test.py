...
def export_attention_test_data(model):
    """Generates input and output data for the first attention block."""
    print("\n--- Generating Attention Block Test Data ---")
    
    # Find the first attention block in the model
    first_attention_block = None
    for module in model.modules():
        if isinstance(module, AttentionBlock):
            first_attention_block = module
            break
            
    if first_attention_block is None:
        print("No AttentionBlock found.")
        return

    # Create a deterministic random input tensor
    torch.manual_seed(0)
    # Get the input channels for the block from its normalization layer
    in_channels = first_attention_block.norm.num_channels
    input_tensor = torch.randn(1, in_channels, 16, 16) # Example size

    # Run the block
    output_tensor = first_attention_block(input_tensor)

    # Save to text files
    np.savetxt("attention_input.txt", input_tensor.flatten().numpy())
    np.savetxt("attention_output_python.txt", output_tensor.flatten().detach().numpy())
    
    print(f"Saved attention_input.txt (shape: {list(input_tensor.shape)})")
    print(f"Saved attention_output_python.txt (shape: {list(output_tensor.shape)})")
    print("------------------------------------------")
# =================================================================================
...
def main():
    ...
    model.eval()
    export_attention_test_data(model)
